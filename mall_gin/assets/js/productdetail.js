...
.then(data => {
    ...
    const product = data[0]; // 假设返回的数据是一个包含单个对象的数组
    ...
    if (typeof product.unit_price !== 'number') {
        console.error("Invalid unit price format.");
        return;
    }

    // 构建商品详情的 HTML 内容，并美化格式
    productDetailDiv.innerHTML = `
        <div class="product-info">
            <h1 class="product-name">商品名称: ${product.product_name}</h1>
            <p class="product-detail-info">商品说明: ${product.product_detail}</p>
            <p class="price">单价: $${product.unit_price.toFixed(2)}</p>
            <p class="category-description">分类: ${product.Category ? product.Category.CategoryDescription : '未定义'}</p>
        </div>
        <div class="buttons">
            <button class="button add-to-cart" onclick="openModal(${product.product_id})">加入购物车</button>
            <button class="button buy-now" onclick="openModal(${product.product_id}, true)">立刻下单</button>
        </div>
    `;
